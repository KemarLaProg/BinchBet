...
// BET
exports.getUserBet = function(uid, callback){
  bdd.db.query("SELECT tb.username, tb.h_goal, tb.a_goal, tg.id_game FROM t_bet AS tb JOIN t_game AS tg ON tb.id_game = tg.id_game JOIN g_games AS gg ON tg.id_game = gg.id_game WHERE tb.username = '"+uid+"'", function (err, result) {
    if(err) throw err;
...
exports.insertGameBet = function(hgoal,agoal,id_game,user,callback){
  var sql_InsertGameBet = "INSERT INTO `t_bet`(`h_goal`, `a_goal`, `username`, `id_game`, `points`) VALUES (?,?,?,?,0)";

  bdd.db.query(sql_InsertGameBet,[ hgoal,agoal,user,id_game], function (err, result) {
    if(err) throw err;
    callback(result);
  });
}
...
exports.lookIfBetExist = function(id_game,user,callback){
  var sql_lookIfBetExist = "SELECT `id_bet` as verif FROM t_bet WHERE t_bet.id_game = ? AND t_bet.username = ?";
  bdd.db.query(sql_lookIfBetExist,[id_game,user], function (err, result) {
    if(err) throw err;
    callback(result);
  });
}
